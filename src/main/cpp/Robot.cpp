...
void Robot::RobotInit() 
{
	...
	_launcher->Periodic(false, 0.0, false, false, false);
	_climb->Periodic(false, false, false, false);
}
...
void Robot::TeleopPeriodic() 
{
	...
	_drivetrain->Drive(SC::F_Deadband(rightDemand, C_DRIVE_DEADBAND),
					   SC::F_Deadband(leftDemand, C_DRIVE_DEADBAND),
					   forceLowGear);
#elif defined(DRIVE_MODE_ARCADE)
	_drivetrain->Drive(SC::F_Deadband(throttleDemand, C_DRIVE_DEADBAND),
					 SC::F_Deadband(turnDemand, C_DRIVE_DEADBAND),
					forceLowGear,
					   GP1_Driver->GetRawButton(C_DRIVER_EBRAKE));
#elif defined(DRIVE_MODE_CURVE)
	_drivetrain->Drive_Curve(SC::F_Deadband(throttleDemand, C_DRIVE_DEADBAND),
							 SC::F_Deadband(turnDemand, C_DRIVE_DEADBAND),
							 forceLowGear);
#endif
	...
	_launcher->Periodic(BB_GameDevice->GetRawButton(C_GD_LAUNCH_AUTO) && _intake->IsCargoLoaded(),
	...
	_climb->Periodic(JS_Climb->GetRawButton(C_CLIMB_STAGE1_EXTEND), JS_Climb->GetRawButton(C_CLIMB_STAGE1_CLAW), 
